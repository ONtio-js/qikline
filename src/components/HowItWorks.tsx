...
const HowItWorks = ({icon,title,description,id}:{icon:React.ReactNode,title:string,description:string,id:number}) => {
  return (
    <motion.div initial={{opacity:0,x:id%2===0?-10:10}} whileInView={{opacity:1,x:0}} transition={{duration:0.3,delay:id*0.1}} whileHover={{scale:1.05}}  className='bg-blue-700/80 md:bg-transparent flex flex-col items-center gap-5 md:max-w-[23rem] group hover:bg-blue-600/80 p-10 transition-all duration-500 ease-in-out rounded-2xl'>
